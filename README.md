...
Перед первым запуском создайте общую сеть для backend и frontend:

`docker network create beatfilm-network`

Собрать и поднять backend:
...
Backend контейнер публикует API на `http://localhost:4200` и также доступен внутри сети `beatfilm-network` по имени `beatfilm-api:4200`.

Чтобы фронтенд из другого `docker-compose.yml` подключился к API, добавьте в его compose подключение к той же внешней сети:
